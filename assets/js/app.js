  ...
  ChartManager.initChart('chart-risk-matrix', {
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        return `${item.type}<br/>概率：${formatPercent(item.prob, 1)}<br/>损失：${formatNumber(item.loss, { digits: 2 })}<br/>等级：${item.level}`;
      }
    },
    xAxis: {
      type: 'category',
    ...
    visualMap: {
      min: 0,
      max: heatData.reduce((max, item) => Math.max(max, item[2]), 0.5),
      calculable: false,
      orient: 'horizontal',
      left: 'center',
      bottom: 0,
